...
def dlt_index():
    """大乐透首页"""
    try:
        latest_results = dlt_db_manager.get_recent_results(1)
        
        if not latest_results:
            return render_template('dlt_index.html', error="暂无数据，请先更新数据")
        
        latest = latest_results[0]
        all_results = dlt_db_manager.get_all_results(order_by='draw_date')
        ...
        for result in all_results:
            formatted_results.append({
                'issue': result['issue'],
                'red_balls': [result['red_1'], result['red_2'], result['red_3'],
                             result['red_4'], result['red_5']],
                'blue_balls': [result.get('blue_1', 0), result.get('blue_2', 0)],
                'draw_date': str(result['draw_date'])
            })
        
        missing_stats = dlt_calculator.calculate_missing_values(formatted_results)
        
        if missing_stats:
            latest_missing = missing_stats[-1]
            ranking = dlt_calculator.get_current_missing_ranking(latest_missing)
            hot_numbers, cold_numbers = dlt_calculator.classify_hot_cold_numbers(latest_missing)
            analysis = dlt_calculator.analyze_issue(latest, latest_missing)
            
            return render_template('dlt_index.html',
                                 latest=latest,
                                 analysis=analysis.to_dict(),
                                 red_ranking=ranking['red_ranking'],
                                 blue_ranking=ranking['blue_ranking'],
            ...
            return render_template('dlt_index.html', latest=latest, error="无法计算遗漏值")
            
    except Exception as e:
        return render_template('dlt_index.html', error=f"加载数据失败：{str(e)}", latest=None)


@app.route('/dlt/history')
def dlt_history():
    """大乐透历史数据"""
    try:
        page = request.args.get('page', 1, type=int)
        ...
        if results_list:
            results_for_calc = results_list.copy()
            missing_stats = dlt_calculator.calculate_missing_values(results_for_calc)
            results_list.reverse()
        ...
        if results_list and missing_stats:
            for i, result in enumerate(paginated_results):
                ...
                if 0 <= forward_idx < len(missing_stats):
                    analysis = dlt_calculator.analyze_issue(result, missing_stats[forward_idx])
                    result['odd_even'] = analysis.red_odd_even
                    result['red_sum'] = analysis.red_sum
                    result['blue_sum'] = analysis.blue_sum
                    result['red_balls_missing'] = analysis.red_missing_values
        else:
            for result in paginated_results:
                result['odd_even'] = '-'
        ...
        return render_template('dlt_history.html',
                             results=paginated_results,
                             current_page=page,
                             total_pages=total_pages,
                             start_page=start_page,
...
@app.route('/dlt/analysis/<issue>')
def dlt_analysis(issue):
    """大乐透单期详情"""
    try:
        all_results = dlt_db_manager.get_all_results(order_by='draw_date')
        ...
        if not target_result:
            return render_template('dlt_analysis.html', error=f"未找到期号 {issue} 的数据")
        ...
        formatted_result = {
            ...
            'red_balls': [target_result['red_1'], target_result['red_2'],
                         target_result['red_3'], target_result['red_4'],
                         target_result['red_5']],
            'blue_balls': [target_result.get('blue_1', 0), target_result.get('blue_2', 0)],
            'draw_date': str(target_result['draw_date'])
        }
        
        missing_stats = dlt_calculator.calculate_missing_values(all_results)
        ...
        if target_missing:
            analysis_result = dlt_calculator.analyze_issue(formatted_result, target_missing)
            
            number_details = []
            for ball in formatted_result['red_balls']:
                idx = formatted_result['red_balls'].index(ball)
                number_details.append({
                    'type': '前区',
                    'number': ball,
                    'missing': analysis_result.red_missing_values[idx]
                })
            
            for i, ball in enumerate(formatted_result['blue_balls']):
                number_details.append({
                    'type': '后区',
                    'number': ball,
                    'missing': analysis_result.blue_missing_values[i]
                })
            ...
            if target_idx > 0:
                ...
                prev_issue_data = {
                    ...
                    'red_balls': [prev_result['red_1'], prev_result['red_2'],
                                 prev_result['red_3'], prev_result['red_4'],
                                 prev_result['red_5']],
                    'blue_balls': [prev_result.get('blue_1', 0), prev_result.get('blue_2', 0)],
                    'draw_date': str(prev_result['draw_date'])
                }
                for stat in missing_stats:
                    if stat.issue == prev_result['issue']:
                        prev_missing_groups = dlt_calculator.calculate_all_red_missing_groups(stat)
                        prev_analysis = {
                            'missing_groups': {str(k): v for k, v in prev_missing_groups.items()}
                        }
                        break
            ...
            if target_idx < len(all_results) - 1 and target_idx >= 0:
                ...
                next_issue_data = {
                    ...
                    'red_balls': [next_result['red_1'], next_result['red_2'],
                                 next_result['red_3'], next_result['red_4'],
                                 next_result['red_5']],
                    'blue_balls': [next_result.get('blue_1', 0), next_result.get('blue_2', 0)],
                    'draw_date': str(next_result['draw_date'])
                }
                for stat in missing_stats:
                    if stat.issue == next_result['issue']:
                        next_missing_groups = dlt_calculator.calculate_all_red_missing_groups(stat)
                        next_analysis = {
                            'missing_groups': {str(k): v for k, v in next_missing_groups.items()}
                        }
                        break
            ...
            return render_template('dlt_analysis.html',
                                 issue_data=formatted_result,
                                 analysis=analysis_result.to_dict(),
                                 number_details=number_details,
                                 prev_issue_data=prev_issue_data,
...
@app.route('/dlt/missing-groups')
def dlt_missing_groups():
    """大乐透历史遗漏分组"""
    try:
        page = request.args.get('page', 1, type=int)
        per_page = 10
        ...
        if not all_results:
            return render_template('dlt_missing_groups.html', error="暂无数据，请先更新数据")
        ...
        issues_with_groups = []
        for i, stat in enumerate(missing_stats):
            missing_groups = dlt_calculator.calculate_all_red_missing_groups(stat)
            result = formatted_results[i] if i < len(formatted_results) else None
        ...
        return render_template('dlt_missing_groups.html',
                             issues=paginated_issues,
                             current_page=page,
                             total_pages=total_pages,
                             start_page=start_page,
        ...
        return render_template('dlt_missing_groups.html', error=f"加载数据失败：{str(e)}")


@app.route('/dlt/api/update_data', methods=['POST'])
@require_password
def dlt_update_data():
    """API: 更新大乐透最新数据"""
    try:
        new_results = dlt_crawler.fetch_latest_data()
        ...
        inserted_count = 0
        for result in new_results:
            if dlt_crawler.validate_result(result):
                dlt_db_manager.insert_lottery_result(result)
                inserted_count += 1
        
        return jsonify({
            'success': True,
            'message': f'成功更新 {inserted_count} 条大乐透数据',
            'count': inserted_count
        })
...
@app.route('/dlt/api/update_all_history', methods=['POST'])
@require_password
def dlt_update_all_history():
    """API: 更新大乐透所有历史数据"""
    try:
        all_results = dlt_crawler.fetch_all_history(max_pages=150)
        ...
        return jsonify({
            'success': True,
            'message': f'成功更新 {len(all_results)} 条大乐透历史数据',
